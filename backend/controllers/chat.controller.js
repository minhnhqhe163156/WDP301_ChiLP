...
const sendChatCustomer = async (req, res) => {
  const session = await mongoose.startSession();
  let committed = false;
  try {
...
const getConversationsSeller = async (req, res) => {
  try {
    ...
    const conversations = await Conversation.find({
      ...
      .populate({
        path: "participants",
        select: "_id name picture_avatar", // Chỉ lấy thông tin cần thiết của người dùng
      })
      .populate({
        path: "lastMessage",
      ...
      conversations.map(async (conv) => {
        ...
        const products = conv.products.map((product) => {
          ...
          return {
            ...
            product_name: productData.title || "Sản phẩm",
        ...
        return {
          conversationId: conv._id,
          customer: otherUser || { name: "Người dùng", picture_avatar: null },
...
const getCustomerConversations = async (req, res) => {
  try {
    ...
    const conversations = await Conversation.find({
      ...
      .populate({
        path: "participants",
        select: "name picture_avatar role",
...
const getConversationMessagesCustomer = async (req, res) => {
  try {
    ...
    await Message.updateMany(
      {
        conversationId: conversationId,
        senderId: { $ne: customerId }, // Messages not sent by the buyer
        status: { $ne: "read" },
      },
      { $set: { status: "read" } }
    );
...
const getConversationProducts = async (req, res) => {
  try {
    ...
    const conversation = await Conversation.findOne({
      ...
      path: "products.productId",
      select: "product_name price images description isAvailable",
    });
