 ...
 */
const trackGetCustomerConversations = (startTime) => {
  const duration = Date.now() - startTime;
  metrics.getCustomerConversations.count++;
  metrics.getCustomerConversations.totalTime += duration;
  metrics.getCustomerConversations.max = Math.max(metrics.getCustomerConversations.max, duration);
  checkSaveMetrics();
};
