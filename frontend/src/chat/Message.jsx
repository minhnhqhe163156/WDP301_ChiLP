...
import ProductBanner, { CustomerProductBanner } from "./ProductBanner";
...
const Message = ({ message, products }) => {
  ...
  if (message.isSystem || message.type === "system") {
    ...
    if (message.id === "system-product" && products && products.length > 0) {
      return <CustomerProductBanner product={products[0]} />;
    }
  ...
  if (message.type === "product-banner") {
    return message.sender === "customer" ? (
      <CustomerProductBanner productRef={message.productRef} />
    ) : (
      <ProductBanner productRef={message.productRef} />
    );
  }

  // Determine if the message is from the customer (current user)
  const isSentByCurrentUser =
    message.sender === "customer" || message.sender === "me";
  ...
  return (
    <MessageBubble
      ...
      showProduct={Boolean(
        message.product || message.productId || message.productRef
      )}
    >
      {/* Text content */}
